...
void Enemy::Move(std::vector<std::vector<Entity*>> Map, float* DeltaT, Player* Player)
{
    ...
    if (m_direction->x != 0)
    {
        ...
        y1 = floor(currentPosition->y + 0.02);
        y2 = ceil(currentPosition->y - 0.02);
    }
    else if (m_direction->y != 0)
    {
        x1 = floor(currentPosition->x + 0.05f);
        x2 = ceil(currentPosition->x - 0.05f);
        y1 = m_direction->y == 1 ? ceil(currentPosition->y - 0.02) : floor(currentPosition->y + 0.02);
        y2 = m_direction->y == 1 ? ceil(currentPosition->y - 0.02) : floor(currentPosition->y + 0.02);
    }
